...
- **项目的技术难点及解决过程**
  ...
  - 联网系统
    - Cocos自带WebSocket延迟太高，不能满足fps游戏的实时同步需求，我们在boost::asio、byrnet和yasio中和选用了更加轻量以及低时延的yasio库处理网络底层连接
    - 实时同步：在客户端中采集玩家的移动和攻击指令，统一上传至服务端处理，再回传给客户端显示。
    
    - 聊天系统：服务端不储存聊天信息，有新消息直接转发给所有客户端。
  - Android端适配和摇杆
