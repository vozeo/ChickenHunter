...
bool Start::init()
{
	...
	auto singleGame = MenuItemFont::create("Singleplayer   ", [=](Ref* render) {
		...
		Director::getInstance()->pushScene(TransitionFade::create(0.3f, Game::create(), Color3B(0, 255, 255)));
		});
	singleGame->setColor(Color3B(255, 215, 0));
	auto multiGame = MenuItemFont::create("   Multiplayer   ", [=](Ref* render) {
		AudioEngine::pause(startAudioID);
		Director::getInstance()->pushScene(TransitionFade::create(0.3f, Server::createScene(), Color3B(0, 255, 255)));
		});
